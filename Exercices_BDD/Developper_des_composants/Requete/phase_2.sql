...
FROM client WHERE cli_nom = 'White';
...
SELECT sta_nom, COUNT(hot_id) AS nombre_hotel
FROM station
...
SELECT sta_nom, COUNT(chambre.cha_id) AS nombre_chambre
FROM station
JOIN hotel ON hotel.hot_sta_id = station.sta_id
...
SELECT sta_nom, AVG(DATEDIFF(res_date_fin, res_date_debut)) AS duree_sejour
FROM station
JOIN hotel ON hotel.hot_sta_id = station.sta_id
JOIN chambre ON chambre.cha_hot_id = hotel.hot_id
JOIN reservation ON reservation.res_cha_id = chambre.cha_id
